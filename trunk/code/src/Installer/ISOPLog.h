		...
		if(INVALID_HANDLE_VALUE == s_hLogFile)
		{
			...
			else if((DWORD)ERROR_ACCESS_DENIED == dwErrCode)
			{
				...
				if(!GetTempPathA(_MAX_PATH, m_szLogFileDirA))
					return;				
#if (defined(UNICODE) || defined(_UNICODE))
				_snprintf_s(m_szLogFileDirA, sizeof(m_szLogFileDirA)/ sizeof(m_szLogFileDirA[0]), _MAX_PATH, "%sTSLOG\\", m_szLogFileDirA);
#else
				_snprintf_s(m_szLogFileDirA, sizeof(m_szLogFileDirA)/ sizeof(m_szLogFileDirA[0]), _MAX_PATH, "%sTSLOG\\", m_szLogFileDirA);
#endif
				DWORD dwPID = GetCurrentProcessId();
...
LPCTSTR CISOPLog::GetCurrentModuleName(void)
{
	...
	pszModuleFileName++;
	_tcsncpy_s(szCurModuleName, pszModuleFileName,_MAX_FNAME); //
